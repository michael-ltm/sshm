...
func handleExec(ctx context.Context, deps Deps, args map[string]any) (any, error) {
	...
	cli, err := dialExecRemote(s, deps.sshOptions(ctx, sshpkg.BuildOpts{ConfigPath: deps.ConfigPath, Alias: alias}))
	if err != nil {
...
func registerExecTools(s *server.MCPServer, deps Deps, names []string) []string {
	execTool := mcp.NewTool("exec",
		mcp.WithDescription("Run in a noninteractive login shell with common POSIX install paths added (profile PATH wins; Windows refreshes user/system PATH; unknown shells unchanged). "+
			"On macOS an explicitly enabled desktop agent supplies existing GitHub credentials only to gh; file operations stay in SSH. A missing CLI or unavailable credential here does not mean the desktop user is logged out. Use check_environment to diagnose. Dangerous commands are blocked unless unsafe=true. "+
