...
func TestNewServer_RegistersExpectedTools(t *testing.T) {
	...
	for _, want := range []string{
		"list_servers", "get_server", "test_connection", "check_ssh", "get_status",
...
func TestNewServer_ReadOnlyOmitsWriteTools(t *testing.T) {
	...
	require.ElementsMatch(t, []string{"list_servers", "get_server", "test_connection", "check_ssh", "get_status"}, names)
	// No write/exec/ops tools.
	for _, forbidden := range []string{"add_server", "edit_server", "remove_server", "exec", "exec_multi", "bootstrap", "gen_key", "copy_id", "tail_logs", "upload", "download", "transfer_start", "transfer_status"} {
