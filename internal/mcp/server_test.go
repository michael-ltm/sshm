...
func TestNewServer_ReadOnlyOmitsWriteTools(t *testing.T) {
	...
	require.ElementsMatch(t, []string{"list_servers", "find_servers", "get_server", "test_connection", "check_ssh", "get_status", "list_projects", "get_project"}, names)
...
func TestRemoveServerSchemaRequiresExactConfirmation(t *testing.T) {
	s, _ := NewServer(Deps{ConfigPath: "/tmp/none.toml", AuditPath: "/tmp/audit.log", AllowWrite: true})
	tool := s.GetTool("remove_server")
	require.NotNil(t, tool)
	require.ElementsMatch(t, []string{"alias", "confirm_alias", "reason"}, tool.Tool.InputSchema.Required)
}
