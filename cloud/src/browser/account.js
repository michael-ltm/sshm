...
function cancelUnlock(){
 ...
 unlockFlow.cancel();$('#action-unlock-form').reset();
...
$('#action-unlock-dialog').addEventListener('close',()=>{if(!$('#action-unlock-dialog').open&&unlockFlow.ticket()!==null)cancelUnlock();});
...
async function logout(){try{await api('web/logout','POST');}catch(e){flash(e.message);return;}session=null;devices=[];lock();navigate('landing',{replace:true});$$('dialog[open]').forEach(d=>d.close());}
...
function install(){
 ...
 $('#link-help').textContent=vault?'安装完成后运行。核对设备验证码，在设备页批准。':'可直接复制命令在设备登录，或点击下方按钮在当前页面解锁，生成由网页批准的接入命令。';
 $('#unlock-install').hidden=!session||!!vault;
}
$('#unlock-install').onclick=()=>ensureVault('生成设备接入命令',()=>install());
...
async function submitUnlock(event){
 ...
 const form=event.currentTarget,error=$('#action-unlock-error'),button=form.querySelector('button.primary');
 ...
 try{
  ...
  $('#action-unlock-dialog').close();
  await continuation;await renderLinks();await refreshJobs();
 }catch(e){if(epoch===unlockEpoch){if(!$('#action-unlock-dialog').open)flash(e.message);else{error.textContent=e.message;form.elements.phrase.focus();}}}
 finally{form.elements.phrase.value='';button.disabled=false;unlocking=false;}
}
$('#unlock-vault').onclick=()=>ensureVault('查看服务器保险库',()=>renderServers());
...
setInterval(async()=>{if(!session||document.hidden)return;try{await api('heartbeat','POST',{platform:'browser',version:'web-preview'});await refresh();}catch(e){if(e.status===401){session=null;devices=[];lock();navigate('devices');flash('登录已过期，请重新登录。');}}},30_000);
...
function openDelete(ids){if(!vault)return ensureVault('删除连接',()=>openDelete(ids));deleteIDs=[...new Set(ids)].filter(id=>vault.data.entries[id]);if(!deleteIDs.length)return;$('#delete-description').textContent='将删除 '+deleteIDs.length+' 个云端连接。';$('#delete-names').replaceChildren(...deleteIDs.slice(0,12).map(id=>node('li','',vault.data.entries[id].aliases[0])));if(deleteIDs.length>12)$('#delete-names').append(node('li','muted','以及另外 '+(deleteIDs.length-12)+' 个连接'));$('#delete-error').textContent='';$('#delete-dialog').showModal();}
...
function openServerTerminal(entry){
 ...
 serverTarget=entry.id;const form=$('#server-terminal-form'),select=form.elements.device;select.replaceChildren();
 const eligible=devices.filter(d=>online(d)&&agents.some(a=>a.device_id===d.id&&a.ssh_targets));
