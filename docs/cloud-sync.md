...
只有 `sshm mcp --cloud-auth browser` 提供 `cloud_unlock(reason)`、`cloud_unlock_status()` 和
`cloud_lock(reason)`。AI 请求解锁后，用户在可信浏览器核对返回的验证码，批准
“MCP credential access”；AI 轮询状态，确认解锁后再执行云端连接操作。
必须已经配置云账号并固定保险库根公钥；工具不接受密码、私钥、恢复码、公钥覆盖或服务地址覆盖。
只读 MCP 不提供这三个工具；`--cloud-auth browser --read-only` 会报错，不能静默退回本地模式。

批准仅属于提出请求的 MCP 进程。CLI 的 `cloud link` 或 `cloud watch`，以及另一个
MCP 进程，都不能为它解锁。不要通过启动 watch 来解决 MCP 的锁定提示。
临时设备令牌和保险库主密钥只保留在该进程内存，不替换 CLI 账号或持久化临时令牌。
请求 10 分钟过期。批准后的默认策略为闲置 2 小时或累计 12 小时后结束访问；
启动 MCP 时可用 `--cloud-idle-timeout` 和 `--cloud-session-max-age` 调整，
持续时间使用 `90m`、`4h`、`720h` 这类格式；两者必须为正数且各自不超过 30 天。
云端临时设备令牌如果更早过期，仍以令牌期限为准。
手动 `cloud_lock`、MCP 重启、设备撤销、账号变更或在线校验失败也会立即结束后续访问，
并尽力撤销临时设备。已经开始的操作可以完成，后续连接必须重新批准。

每次新连接都在线验证授权和签名保险库内容，无离线回退。账号或根公钥改变、
设备被撤销、签名无效或版本回退都会使会话失效。连接绑定必须与保险库内的账号、
条目及路线一致；缺失、冲突、已删除、密码不明确或加密私钥需由用户在聊天之外修正。
在线快照通过签名验证并成功解锁后即视为一次活动，即使随后发现条目、路线或凭据问题；
仅查询状态、网络失败以及账号、保险库身份或签名校验失败不会刷新闲置期限，累计期限也不会刷新。
不回退到本地密钥或 Agent。只支持保险库拥有的简单 ProxyJump，
不支持嵌套跳板、外部 ProxyCommand、转发、SOCKS 或设备终端条目。

批准授予整个 MCP 进程完整保险库凭据访问权，并沿用现有 AI 命令权限，
不是只批准某台主机或某条命令。主机密钥验证、命令过滤、审计及结果脱敏仍有效；
内存清理在 Go 中只能尽力而为，不能防止同用户进程检查内存，也不能保证已授权的
远程命令不会读取并返回远端秘密。
